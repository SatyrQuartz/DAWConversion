...
parser.add_argument("lmmsIN")
parser.add_argument("ConvProjOUT")
...
def lmms_getvalue(defualt, jsonvar, xmltag, xmlname, json_name):
	...
	if headxml.get(xmlname) != None:
		jsonvar[json_name] = float(headxml.get(xmlname))
...
def lmms_decodeplugin(trackxml_insttr, jsonplugindata, instrumentdata_json):
	...
	if pluginname == "audiofileprocessor":
		instrumentdata_json['plugin'] = "sample"
...
tree = ET.parse(args.lmmsIN).getroot()
...
with open(args.ConvProjOUT, 'w') as outfile:
